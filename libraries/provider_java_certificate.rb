...
#
# Cookbook Name:: java_libs
# Provider:: java_certificate
#
...
class Chef
  class Provider
    class JavaCertificate < Chef::Provider::LWRPBase
      ...
      action :install do
        ...
        truststore = ::File.join(node['java']['java_home'], 'jre/lib/security/cacerts') if truststore.nil?
        ...
        if certdata.nil?
          if !certdatafile.nil?
            ...
            if $?.success?
              ...
              Chef::Application.fatal!("Error returned when attempting to retrieve certificate from remote endpoint #{certendpoint}: #{$?}", $?.to_s[/exit (\d+)/, 1].to_i)
            end
          else
        ...
        if ::File.exist?(certfile)
          ...
          if has_key
            ...
            Chef::Application.fatal!("Error deleting existing certificate \"#{certalias}\" in keystore so it can be updated: #{$?}", $?.to_s[/exit (\d+)/, 1].to_i) unless $?.success?
          end
          ...
          Chef::Log.debug("Executing: #{keytool} -import -trustcacerts -alias \"#{certalias}\" -file #{certfile} -keystore #{truststore} -storepass #{truststore_passwd} -noprompt\n#{result}")
      ...
      action :remove do
        ...
        truststore_passwd = 'changeit' if truststore_passwd.nil?
        keytool = ::File.join(node['java']['java_home'], 'bin/keytool')
